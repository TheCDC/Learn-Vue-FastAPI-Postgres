...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def remove_multi(self, db: Session, *, ids: List[int]) -> List[ModelType]:
        obj = db.query(self.model).filter(self.model.id.in_(ids)).delete()
        db.commit()
        return obj

    def user_can_read(self, db: Session, id: int, user: models.User) -> bool:
        raise NotImplementedError(
            f"user_can_read not implemented for class {self.__class__.__name__}"
        )

    def user_can_write(self, db: Session, id: int, user: models.User) -> bool:
        raise NotImplementedError(
            f"user_can_write not implemented for class {self.__class__.__name__}"
        )
