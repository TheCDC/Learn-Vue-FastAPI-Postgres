...
def test_create_bekpacktrip(client: TestClient, db: Session) -> None:
    ...
    trip = client.get(
        f"{settings.API_V1_STR}/bekpack/bekpacktrips/{trip_id}", headers=auth_token,
    )
    content = trip.json()
    assert "id" in content
...
def test_get_bekpacktrip_mine_multi(client: TestClient, db: Session) -> None:
    ...
    response = client.get(
        f"{settings.API_V1_STR}/bekpack/bekpacktrips/mine/all", headers=token_headers,
    )
    content = response.json()
...
def test_update_bekpacktrip(
    ...
    # then delete user
    response = client.delete(
        f"{settings.API_V1_STR}/bekpack/bekpackusers/{user_id_old}", headers=auth_token,
    )
