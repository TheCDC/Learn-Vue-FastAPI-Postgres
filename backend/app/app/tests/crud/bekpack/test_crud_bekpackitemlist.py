...
def test_create_bekpackitemlist(db: Session) -> None:
    ...
    trip = create_random_trip(db=db, bekpack_user=bp_user)
    ...
    itemlist = crud.bekpackitemlist.create_with_trip_owner(
        ...
        parent_user_id=bp_user.owner.id,
        parent_trip_id=trip.id,
    )
    assert itemlist
    assert itemlist.parent_user_id == bp_user.id
    assert itemlist.parent_trip_id == trip.id


def test_itemlist_get_by_owner(db: Session):
    bp_user = create_random_bekpackuser(db=db)
    objects_created = [
        create_random_itemlist(db=db, bekpack_user=bp_user) for _ in range(10)
    ]
    objects_found = crud.bekpackitemlist.get_by_owner(db=db, owner_id=bp_user.owner.id)
    print(
        schemas.BekpackUser.from_orm(bp_user),
        [schemas.BekpackItemList.from_orm(i) for i in objects_created],
        [schemas.BekpackItemList.from_orm(i) for i in objects_found],
        sep="\n",
    )
    assert len(objects_created) == len(objects_found)
    assert all(i.parent_user_id == bp_user.id for i in objects_found)
    target_ids = set([i.id for i in objects_created])
    found_ids = set([i.id for i in objects_found])
...
def test_itemlist_get_by_trip(db: Session):
    ...
    itemlists = [
        create_random_itemlist(db=db, trip=trip, bekpack_user=trip.owner)
        for _ in range(5)
    ]
