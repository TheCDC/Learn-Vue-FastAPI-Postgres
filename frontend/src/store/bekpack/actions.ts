...
export const actions = {
  async actionCreateUser(context: MainContext) {
    try {
      const response = await api.createBekpackUser(
  ...
  async actionGetMyTrips(context: MainContext, payload: { page: IPageRead }) {
    try {
      const response = await api.getMyBekpackTrips(
        context.rootState.main.token,
        payload.page
      );
      if (response) {
  ...
  async actionGetTripToEdit(context: MainContext, payload: { id: number }) {
    try {
      const response = await api.getBekpackTrip(
        context.rootState.main.token,
        payload.id
      );
      if (response) {
  ...
  async actionUpdateTrip(
    context: MainContext,
    payload: { id: number; item: IBekpackTripUpdate }
  ) {
    try {
      const loadingNotification = { content: "saving", showProgress: true };
      commitAddNotification(context, loadingNotification);
  ...
  async actionDeleteTrip(context: MainContext, payload: { id: number }) {
    try {
      ...
      const responseRead = await api.getMyBekpackTrips(
        context.rootState.main.token,
        context.state.trips
      );
